from neo4j import Driver, Result, Record
import pytest
...
def test_add_person_creates_node(neo4j_session):
    ...
    # Cleanup
    neo4j_session.run("MATCH (p:Person {name: $name}) DETACH DELETE p", name=person_name)

    neo4j_session.execute_write(add_person, person_name)

    result: Result = neo4j_session.run("MATCH (p:Person {name: $name}) RETURN p", name=person_name)
